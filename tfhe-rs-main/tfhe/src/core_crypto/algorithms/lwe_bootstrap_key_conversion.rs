    ...
    OutputCont: ContainerMut<Element = u64>,
{
    ...
    // In the following: I commented "let num_threads = rayon::current_num_threads();" and
    // "//let chunk_size = input_as_polynomial_list
    //    .polynomial_count()
    //    .0
    //    .div_ceil(num_threads);"
    // and replaced them with "let chunk_size = 1;"
    ...
    // In the following: I commented "input_as_polynomial_list
    ...
    //        }
    //    });"
    // and replaced it with "for (input_poly_chunk, mut output_poly_chunk) in input_as_polynomial_list
    //    .chunks(chunk_size)
    //    .zip(output_as_polynomial_list.chunks_mut(chunk_size))
    //{
    //    for (input_poly, output_poly) in
    //        input_poly_chunk.iter().zip(output_poly_chunk.iter_mut())
    //    {
    ...
    for (input_poly_chunk, mut output_poly_chunk) in input_as_polynomial_list
        .chunks(chunk_size)
        .zip(output_as_polynomial_list.chunks_mut(chunk_size))
    {
        for (input_poly, output_poly) in
